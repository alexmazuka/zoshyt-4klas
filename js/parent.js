...
(async function () {
  ...
  async function detail(id) {
    ...
    const aiBlock = (aiAsks.length || aiOpens) ? `<div class="card" style="background:#faf5ff"><h3 style="margin-top:0">🦉 Поясняйко</h3><p class="muted">Відкривав чат: ${aiOpens} раз(ів) · Запитань поставлено: ${aiAsks.length}</p>${aiAsks.map(e => `<div class="ans-row"><div class="q">🧒 ${Z.md(e.q)} <small class="muted">(крок: ${STEP_UA[e.step] || e.step || '?'})</small></div><div class="child">🦉 ${Z.md(e.a)}</div><small class="muted">${Z.fmtDT(e.t)}</small></div>`).join('')}</div>` : '';
